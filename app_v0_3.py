...
def sleeper_get(path):
    req = urllib.request.Request(
        API + path,
        headers={"User-Agent": "WoRP-Lab/0.3"}
    )
...
st.title("WoRP Lab")
st.caption("USER → LEAGUE → GRAPH · Engine V0.2.1 frozen · UI V0.3")
...
if not user:
    ...
    st.info("V0.3 integration gate: USER → LEAGUES → LEAGUE FORMAT → ROSTERS. WoRP roster calculations are not enabled until this data path is validated.")
    st.stop()
...
st.info(
    "Sleeper connection is live. V0.3 adds the validated historical WoRP shape as a reference curve. "
    "It does not relabel that curve as league-specific: this selected league still needs a validated "
    "Sleeper → scoring → player-universe calculation before custom WoRP values are shown."
)

curve = build_three_year_curve(history)
if not curve.empty:
    st.markdown("#### 3-Year WoRP Avg — reference shape")
    st.caption("2023–2025 · Frozen Engine V0.2.1 reference results · positional rank 1–50. Use this as a qualitative Scott Connor-style sanity check, not as Scott's WAR data.")
    try:
        import altair as alt
        domain = ["QB", "RB", "WR", "TE"]
        colors = ["#ff4b4b", "#21c56e", "#3b82f6", "#f5b301"]
        chart = (alt.Chart(curve)
            .mark_line(point=alt.OverlayMarkDef(size=34), strokeWidth=2.2)
            .encode(
                x=alt.X("position_rank:Q", title="Position Rank", scale=alt.Scale(domain=[1,50]), axis=alt.Axis(values=list(range(1,51,2)))),
                y=alt.Y("three_year_worp_avg:Q", title="3-Year WoRP Avg", scale=alt.Scale(zero=False)),
                color=alt.Color("position:N", title=None, scale=alt.Scale(domain=domain, range=colors), legend=alt.Legend(orient="top", direction="horizontal")),
                tooltip=[alt.Tooltip("position:N", title="Pos"), alt.Tooltip("position_rank:Q", title="Rank"), alt.Tooltip("player_name:N", title="Player"), alt.Tooltip("three_year_worp_avg:Q", title="3Y WoRP Avg", format=".3f")],
            )
        ...
        st.warning(f"Reference graph could not render: {e}")

    with st.expander("Reference curve data"):
        table = curve[["position","position_rank","player_name","three_year_worp_avg"]].copy()
        table.columns = ["Pos", "Pos Rank", "Player", "3Y WoRP Avg"]
        st.dataframe(table, hide_index=True, width="stretch")
else:
    st.caption("Historical CSV not found or missing required columns; reference curve unavailable.")

st.markdown("### Rosters detected")
st.dataframe(roster_df, hide_index=True, width="stretch")

st.caption(
    "V0.3 is an integration + benchmark build. Historical WoRP remains frozen; league-specific WoRP and the final roster graph "
