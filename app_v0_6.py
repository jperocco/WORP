...
st.title("WoRP Lab")
st.caption("USER → LEAGUE → GRAPH · Engine V0.2.1 frozen · UI V0.6")
...
if not user:
    ...
    st.info("V0.6: league-specific market curve + player board, powered by validated Sleeper-native scoring.")
    st.stop()
...
st.caption(
    ...
    "3-Year value curve and player board."
)
...
if not has_result:
    ...
    if not rank1.empty:
        ...
        signal_a.metric(
            "Highest elite premium",
            f"{top_row['position']}1",
            f"{top_row['three_year_worp_avg']:.2f} WoRP",
        )
    else:
    ...
    if not rank12.empty:
        ...
        signal_b.metric(
            "Strongest depth at Pos12",
            depth_row["position"],
            f"{depth_row['three_year_worp_avg']:.2f} WoRP",
        )
    else:
    ...
    if not cliff_df.empty:
        ...
        signal_c.metric(
            "Sharpest 5-rank cliff",
            f"{biggest_cliff['position']}{int(biggest_cliff['cliff_start_rank'])}",
            f"-{biggest_cliff['worp_drop']:.2f} WoRP",
        )
    else:
    ...
    signal_d.metric(
        "Window",
        window_label,
        f"{int(n_sims_ui):,} sims".replace(",", "."),
    )

    tab_curve, tab_players, tab_method = st.tabs(
        ["MARKET CURVE", "PLAYER BOARD", "METHODOLOGY"]
    )

    with tab_curve:
        st.markdown("#### 3-Year WoRP Avg — positional market curve")
        st.caption(
            f"{window_label} · each season is scored with this league's exact "
            "Sleeper scoring. The chart averages WoRP at each positional rank."
        )
        ...
        try:
            ...
            st.warning(f"Market curve could not render: {e}")
    ...
    with tab_players:
        ...
        board_view = board[display_cols].rename(columns={
            ...
            "position": "Pos",
            "player_name": "Player",
            "three_year_worp_avg": "3Y Avg WoRP",
            "seasons": "Seasons",
        ...
        for col in ["3Y Avg WoRP", "Avg Pos Rank", f"{max(selected_seasons)} WoRP", "Avg WoRP/G", "Avg PORP"]:
    ...
    with tab_method:
        ...
        st.write(
            "**WoRP estimates how many fantasy matchup wins a player added "
            "compared with a replacement-level player in your league format.**"
        )
        st.write(
        ...
        st.write(
            "**3-Year Market Curve** ranks players within position in each "
            "season, then averages the WoRP attached to each positional rank. "
            "It describes the positional market shape, not a single player's "
...
st.caption(
    "WoRP Lab UI V0.6 · Sleeper-native scoring · Engine V0.2.1 frozen."
)
