...
st.title("WoRP Lab")
st.caption("USER → LEAGUE → GRAPH · Engine V0.2.1 frozen · UI V0.6.2")
...
if not user:
    ...
    st.info("V0.6.2: concept-first positional curve + interactive focus + automatic structural insights.")
    st.stop()
...
def calculate_league_intelligence_cached(
    ...
    # Deterministic presentation-layer insights from the historical curve.
    insights = []

    def _value_at(pos, rank):
        row = curve[
            (curve["position"] == pos)
            & (curve["position_rank"] == int(rank))
        ]
        if row.empty:
            return None
    ...
    for pos in ["QB", "RB", "WR", "TE"]:
        ...
        if v1 is not None and v12 is not None:
            elite_rows.append((pos, v1 - v12, v1, v12))
    if elite_rows:
        pos, gap, v1, v12 = max(elite_rows, key=lambda x: x[1])
        insights.append({
            "kind": "elite_concentration",
            "title": f"{pos} has the strongest elite concentration",
            "detail": (
                f"{pos}1 averages {v1:.2f} WoRP versus {v12:.2f} at {pos}12 "
                f"(gap {gap:.2f})."
            ),
        })
    ...
    for pos in ["QB", "RB", "WR", "TE"]:
        v12 = _value_at(pos, 12)
        if v12 is not None:
            depth_rows.append((pos, v12))
    if depth_rows:
        pos, v12 = max(depth_rows, key=lambda x: x[1])
        insights.append({
            "kind": "depth",
            "title": f"{pos} retains the most value through {pos}12",
            "detail": f"{pos}12 averages {v12:.2f} WoRP in the selected window.",
        })
    ...
    if not cliff_df.empty:
        ...
        insights.append({
            "kind": "cliff",
            "title": (
                f"Sharpest cliff: {r['position']}{int(r['cliff_start_rank'])}"
                f"→{int(r['cliff_end_rank'])}"
            ),
            "detail": (
                f"Average WoRP falls {float(r['worp_drop']):.2f} across those five ranks."
            ),
        })
    ...
    for pos in ["QB", "RB", "WR", "TE"]:
        x = curve[
            (curve["position"] == pos)
            & (curve["three_year_worp_avg"] > 0)
        ]
        if not x.empty:
            idx = x["position_rank"].idxmax()
            positive_depth.append(
                (
                    pos,
                    int(x.loc[idx, "position_rank"]),
                    float(x.loc[idx, "three_year_worp_avg"]),
                )
            )
    if positive_depth:
        pos, rank, value = max(positive_depth, key=lambda x: x[1])
        insights.append({
            "kind": "positive_depth",
            "title": f"{pos} stays above replacement deepest",
            "detail": f"The curve remains above 0 WoRP through {pos}{rank} ({value:.2f}).",
        })
    ...
    for pos in ["QB", "RB", "WR", "TE"]:
        ...
        v12 = _value_at(pos, 12)
        if v1 is not None and v12 is not None:
            flat_rows.append((pos, v1 - v12, v1, v12))
    if flat_rows:
        pos, gap, v1, v12 = min(flat_rows, key=lambda x: x[1])
        insights.append({
            "kind": "flatness",
            "title": f"{pos} is the flattest top-12 position",
            "detail": (
                f"{pos}1 to {pos}12 declines only {gap:.2f} WoRP "
                f"({v1:.2f} → {v12:.2f})."
            ),
        })
...
if not has_result:
    ...
    with tab_curve:
        ...
        try:
                ...
                .encode(
                    ...
                    color=alt.Color(
                        ...
                        legend=alt.Legend(
                            ...
                            title="Click to focus · shift-click for multiple",
                        ),
                    ),
            ...
            st.caption(
                "Tip: click a position in the legend to focus it. "
                "Shift-click keeps multiple positions in focus; click blank chart space to reset."
            )
        ...
        st.caption(
            "Automatic descriptions of the historical positional curve only. "
            "They summarize what happened in the selected window; they are not projections."
        )

        if insight_df is not None and not insight_df.empty:
            for _, insight in insight_df.iterrows():
                st.markdown(
                    "**{}**  \n{}".format(
                        insight["title"],
                        insight["detail"],
    ...
    with tab_method:
        ...
        st.info(
            "The WoRP Engine V0.2.1 mathematics are unchanged. V0.6.2 changes "
...
st.caption(
    "WoRP Lab UI V0.6.2 · Sleeper-native scoring · Engine V0.2.1 frozen."
)
