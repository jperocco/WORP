...
from worp_roster_construction_product_v0_9_4 import recover_v0_32_ranges, roster_construction_envelope, whole_roster_layers
    ...
    with tab_curve:
        ...
        try:
            ...
            for lr in lineup_rows:
                ...
                mix=(''.join(f'<span>{p} {100*v:.0f}%</span>' for p,v in sorted(occ.items(),key=lambda kv:kv[1],reverse=True) if v>=0.005) if is_flexible else '')
                cards.append(
        ...
        try:
            ...
            active_roster_size=start_n+int(counts.get("BN",0))+int(counts.get("BENCH",0))
            rc_path=Path("worp_roster_construction_product_ranges_v0_32.csv")
            v24_path=Path("worp_scoring_core_decision_equivalence_envelopes_v0_24.csv")
            rc_raw=pd.read_csv(rc_path) if rc_path.exists() else pd.DataFrame()
            v24=pd.read_csv(v24_path) if v24_path.exists() else pd.DataFrame()
            rc=recover_v0_32_ranges(rc_raw,v24)
            envelope=roster_construction_envelope(
                curve=st.session_state["league_curve"].copy(),
                historical_ranges=rc,
                teams=_teams,qb=_qb,rb=_rb,wr=_wr,te=_te,
                flex=_flex,superflex=_superflex,
                active_roster_size=active_roster_size,tep=tep,
            )
            layers=whole_roster_layers(envelope,active_roster_size,_superflex)
            primary=" + ".join(layers["primary_optionality"])
            secondary=" + ".join(layers["secondary_optionality"])
            optionality_priority=f"Primary: {primary}"+(f" · Secondary: {secondary}" if secondary else "")
            st.markdown(
                f"**Active roster: {layers['active_roster_size']}**  \n"
                f"**Scoring Core: {envelope['scoring_core_low']}–{envelope['scoring_core_high']}**  \n"
                f"QB {envelope['QB_low']}–{envelope['QB_high']} · "
                f"RB {envelope['RB_low']}–{envelope['RB_high']} · "
                f"WR {envelope['WR_low']}–{envelope['WR_high']} · "
                f"TE {envelope['TE_low']}–{envelope['TE_high']}  \n"
                f"**Optionality capacity: {layers['optionality_low']}–{layers['optionality_high']}**  \n"
                f"{optionality_priority} · WR deprioritized · TE unresolved"
            )
            st.caption(
                f"Complete active-roster accounting: {envelope['scoring_core_low']} Scoring + {layers['optionality_high']} Optionality = {layers['active_roster_size']}; "
                f"{envelope['scoring_core_high']} Scoring + {layers['optionality_low']} Optionality = {layers['active_roster_size']}. "
                "Optionality evidence is directional, not an exact positional quota: QB material hits recurred in 3/3 seasons, RB in 2/3, WR had zero at the validated hurdle, and TE remained unresolved."
            )
            if envelope["source"]=="EXACT_HISTORICAL_SUPPORT":
                st.caption("Empirically validated for this exact historical format. Independent positional bounds are decision-equivalent constraints, not additive quotas.")
            else:
                st.caption("League-native derived envelope · lower validation. Calculated from this league's team count, slot eligibility, active roster capacity and WoRP curve, then restricted to the V0.24 decision-equivalent plateau — not from Wookiee or a nearest-format substitute. Positional bounds are non-additive.")
            st.caption("Once meaningful Scoring capacity is satisfied, every remaining active-roster slot is explicitly assigned to Non-Scoring optionality capacity.")
        except Exception as exc:
