...
ui=r'''        st.markdown("##### Lineup Share")
        ...
        try:
            ...
            for lr in lineup_rows:
                ...
                cards.append(f'''<div class="ls-card"><div class="ls-slot">{lr['slot']}</div><div class="ls-share">{100*lr['lineup_share']:.1f}%</div><div class="ls-pts">{lr['points_week']:.1f} pts/week</div>{mix}</div>''')
            st.markdown('''<style>
            ...
            st.caption("Historical 2023–25 average under this league's exact Sleeper scoring. Share is fantasy-point contribution to the expected starting lineup; flexible-slot mix is observed positional occupancy in the scoring model. WoRP is not used in this panel.")
