#!/usr/bin/env python3
"""
WOOKIEE INTERPOSITIONAL FREE VALUE + VOLUME AUDIT V0.2.2
...
print("="*112)
print("WOOKIEE INTERPOSITIONAL FREE VALUE + VOLUME AUDIT V0.2.2")
print("="*112)
...
print(f"Sleeper -> GSIS bridge — all cache rows: {w['gsis_id'].notna().sum()}/{len(w)} = {100*bridge_rate:.1f}%")

# The cache contains many zero-FP / economically irrelevant rows.
# Gate the bridge on the universe that can actually affect this study.
fp = pd.to_numeric(w["fantasy_points"], errors="coerce").fillna(0.0)
econ = (fp != 0) | (pd.to_numeric(w["weekly_worp"], errors="coerce").fillna(0.0) > 0)
econ_n = int(econ.sum())
econ_ok = int((w.loc[econ, "gsis_id"].notna()).sum())
econ_rate = econ_ok / max(1, econ_n)
print(f"Sleeper -> GSIS bridge — economic rows: {econ_ok}/{econ_n} = {100*econ_rate:.1f}%")

# Most important loss check: positive WoRP rows with no bridge.
pos = pd.to_numeric(w["weekly_worp"], errors="coerce").fillna(0.0) > 0
pos_n = int(pos.sum())
pos_missing = w.loc[pos & w["gsis_id"].isna()].copy()
pos_missing_rate = len(pos_missing) / max(1, pos_n)
print(f"Positive-WoRP rows missing bridge: {len(pos_missing)}/{pos_n} = {100*pos_missing_rate:.2f}%")

if len(pos_missing):
    print("\nTop missing positive-WoRP rows:")
    print(pos_missing.sort_values("weekly_worp", ascending=False)[
        ["season","week","player_id","player_name","position","fantasy_points","weekly_worp"]
    ].head(20).to_string(index=False))

# 4% relevance trigger: do not bury material missingness.
if pos_missing_rate >= .04:
    print("\nGATILHO 4%: >=4% of positive-WoRP rows lack the ID bridge.")
    print("STOP for relevance/alignment before interpreting economics.")
    raise SystemExit(2)

print("Bridge gate: PASS for positive-WoRP economic universe (<4% missing).")
...
m.to_csv("wookiee_interpositional_free_value_volume_detail_v0_2_2.csv",index=False)
...
]).to_csv("wookiee_interpositional_free_value_volume_by_position_v0_2_2.csv",index=False)
...
cases.to_csv("wookiee_interpositional_free_value_human_cases_v0_2_2.csv",index=False)
