...
It intentionally does not declare 0.05 vs 0.07 WoRP meaningful. Instead it exposes
absolute future-impact distributions and tests multiple material hurdles. The hurdles
are sensitivity landmarks, not semantic truth; the goal is to see whether positional
conclusions survive when we restrict attention to genuinely large outcomes.
...
# Absolute four-week WoRP hurdles. We do not choose a single winner in advance.
...
# Distribution first: let the data show the scale of meaningful tails.
dist=[]
for pos in POSITIONS:
    ...
    for metric in ['future_positive_worp','future_captured_positive_worp']:
        ...
        dist.append({
            'position':pos,'metric':metric,'n_events':len(s),
            'p50':s.quantile(.50),'p75':s.quantile(.75),'p85':s.quantile(.85),
...
for pos in POSITIONS:
    ...
    for hurdle in HURDLES:
        # Produced-tail event: sporting value existed after the shock.
        prod=x[x.future_positive_worp>=hurdle]
        # Captured-tail event: manager actually captured a material block in lineup.
        cap=x[x.future_captured_positive_worp>=hurdle]
        rows.append({
            'position':pos,'four_week_worp_hurdle':hurdle,
            'pool_player_weeks':den,'full_horizon_shocks':len(x),
            'produced_big_hits':len(prod),
            'produced_big_hits_per_100_pool_weeks':100*len(prod)/den,
            'produced_big_hit_share_of_shocks':len(prod)/len(x),
            'captured_big_hits':len(cap),
            'captured_big_hits_per_100_pool_weeks':100*len(cap)/den,
            'captured_big_hit_share_of_shocks':len(cap)/len(x),
            'capture_conversion_of_big_produced_hits':len(x[(x.future_positive_worp>=hurdle)&(x.future_captured_positive_worp>=hurdle)])/len(prod) if len(prod) else np.nan,
            'total_future_worp_from_produced_big_hits':prod.future_positive_worp.sum(),
            'total_captured_worp_from_captured_big_hits':cap.future_captured_positive_worp.sum(),
            'median_future_worp_when_big_hit':prod.future_positive_worp.median() if len(prod) else np.nan,
            'median_captured_worp_when_big_hit':cap.future_captured_positive_worp.median() if len(cap) else np.nan,
        })
